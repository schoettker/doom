...
;; Gruvbox customization — only applied when gruvbox is the active theme.
;; The gruvbox-theme package is declared in packages.el but deferred;
;; it will be loaded automatically when you run (load-theme 'gruvbox t).
...
(defun customize-gruvbox ()
  ...
  (if (member 'gruvbox custom-enabled-themes)
      (custom-theme-set-faces
       'gruvbox
       '(cursor                 ((t (:foreground "#fa7d00" :background "#fa0032"))))
       ...
       '(mu4e-unread-face                  ((t (:foreground "#fbf1c7" :weight bold))))
